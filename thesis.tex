...
\subsection{Loggers}
Log messages are created by loggers. Definition of a logger consists of a category which is represented by package or class name. Whenever the logger tries to log a message, it first checks if the level of the message is greater than level of the logger. After evaluating the level condition, a filter determines whether the log message is loggable or not. Detailed description of all filters can be found in the Logging documentation of Wildfly \cite[Logging Configuration]{wildfly_doc}.
...
\subsection{Handlers}
Handlers represent next step in the log processing. If a log message is loggable, handler processes the message e.g. sends the message to system console or stores the message to a file.
...
Logging subsystem provides straightforward process of custom handler implementation. A custom logger handler is standard module in the JBoss Modules architecture \ref{jboss_modules_sec}.

The simplest way how to create a custom handler is to extend \textit{java.util.logging.Handler} class which is part of \textit{Java Logging API} and provides standard log handler lifecycle.
