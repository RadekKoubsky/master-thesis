...
This chapter briefly describes Elasticsearch and Kibana technologies which are used for storage and visualization of data. In the context of this work, the data represents log messages from Wildfly Application Server \ref{wildfly_chapter}.
...
\subsection{Accessing Elasticsearch}
Connecting to Elasticsearch cluster can be done in several ways depending on user requirements. There are two main approaches for working with Elasticsearch: \textit{RESTful API} and \textit{Java API}.

The Java API provides two built-in clients which directly communicate with Elasticsearch using the native \textit{Elasticsearch transport protocol}.

\subsubsection{RESTful API}
Elasticsearch provides comprehensive RESTful API which is used by the command line interface and web clients. RESTful API uses HTTP protocol and JSON as a data format, the default port for communication is \textit{9200}. 
...
Another use of the RESTful API are web clients. The web clients are built on the RESTful API and provides access to Elasticsearch in various languages such as .NET, JavaScript, Python and many others.
