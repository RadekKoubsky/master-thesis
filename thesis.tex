...
The only requirement for the \textit{CustomHelper} is to extend base \textit{Helper} class. The \textit{CustomHelper} automatically inherits all built-in methods from the base \textit{Helper} if the methods are not overridden. A new method in the \textit{CustomHelper} must have \textit{public} access level modifier. Thus, a rule can use methods exposed the \textit{CustomHelper}.
...
A \textit{CustomHelper} bound to a set of rules without helper class is placed before the first rule definition in the set. The \textit{CustomHelper} is applied to all rules in the se until the first occurrence of another \textit{CustomHelper}.
