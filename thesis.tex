...
\subsection{Cluster}
A \textit{Cluster} consists of one or more nodes (servers) that together stores all data. Unified indexing and search operations are performed across all nodes in a cluster. Every cluster has its own name which is unique, default value of the name is \verb|elasticsearch|.

\subsection{Node}
A node represents a server within a cluster. A node is responsible for storing, indexing and searching operations in the context of a cluster that the node belongs to. A node is identified by name which has to be unique.
